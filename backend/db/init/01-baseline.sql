...
CREATE TABLE `channel_mapping` (
  ...
  `updated_by` bigint unsigned DEFAULT NULL,
  `deleted_at` datetime DEFAULT NULL,
  `source_medium_active` varchar(400) GENERATED ALWAYS AS (if(`deleted_at` is null,concat(`source`,_utf8mb4'|',`medium`),NULL)) VIRTUAL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_source_medium_active` (`source_medium_active`),
  KEY `idx_deleted` (`deleted_at`),
...
CREATE TABLE `roles` (
  ...
  `updated_by` bigint unsigned DEFAULT NULL,
  `deleted_at` datetime DEFAULT NULL,
  `name_active` varchar(100) GENERATED ALWAYS AS (if(`deleted_at` is null,`name`,NULL)) VIRTUAL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_name_active` (`name_active`),
