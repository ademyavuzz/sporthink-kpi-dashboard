...
export default function ChannelMappingPage() {
  ...
  const canUpdate = has("mappings.update");
  const canDelete = has("mappings.delete");
  const [open, setOpen] = useState(false);
  const [pendingEdit, setPendingEdit] = useState<ChannelMappingItem | null>(
    null,
  );
  const [pendingDelete, setPendingDelete] = useState<ChannelMappingItem | null>(
    null,
  );
  ...
  const createMut = useMutation({
    mutationFn: (p: ChannelMappingCreate) => adminApi.createChannelMapping(p),
    onSuccess: () => {
      setOpen(false);
  ...
  const updateMut = useMutation({
    mutationFn: (vars: { id: number; payload: ChannelMappingUpdate }) =>
      adminApi.updateChannelMapping(vars.id, vars.payload),
    onSuccess: () => {
      setPendingEdit(null);
      void qc.invalidateQueries({ queryKey: ["channel-mappings"] });
    },
          ...
          {q.isPending ? (
            ...
            total === 0 ? (
              <EmptyState
                ...
                actionLabel={canCreate ? t("admin:channels.empty_cta") : undefined}
                  ...
                  {filtered.map((m, idx) => (
                    ...
                    >
                      ...
                      <TableCell className="px-3 py-3.5 text-xs text-text-muted">
                        ...
                        >
                          {m.notes ?? <span className="text-text-dim">—</span>}
                        </span>
                      </TableCell>
                      <TableCell className="px-3 py-3.5">
                        {m.is_auto_assigned ? (
                        ...
                        <div className="flex items-center justify-end gap-1">
                          {canUpdate && (
                            <Button
                              size="icon"
                              variant="ghost"
                              onClick={() => setPendingEdit(m)}
                              aria-label={t("admin:channels.edit_title")}
                              title={t("admin:channels.edit_title")}
                              className="size-8 text-text-muted hover:bg-primary/10 hover:text-primary"
                            >
        ...
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-start gap-3">
              ...
              <span className="flex flex-col gap-0.5">
                <span className="text-base font-semibold text-foreground">
                  {t("admin:channels.edit_title")}
                </span>
                <span className="text-xs font-normal text-text-muted">
                  {t("admin:channels.edit_subtitle")}
                </span>
              </span>
            </DialogTitle>
          </DialogHeader>
          {pendingEdit && (
            <ChannelMappingEditForm
              mapping={pendingEdit}
              onSubmit={(payload) =>
                updateMut.mutate({ id: pendingEdit.id, payload })
              }
...
function ChannelMappingCreateForm({
  onSubmit,
  loading,
  onCancel,
}: {
  onSubmit: (p: ChannelMappingCreate) => void;
  loading: boolean;
  onCancel: () => void;
}) {
  const { t } = useTranslation(["admin", "common"]);
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <FieldShell
          icon={Globe}
          label={t("admin:channels.field_source")}
          htmlFor="src"
        >
          <Input
            id="src"
            ...
            placeholder={t("admin:channels.field_source_placeholder")}
            required
            autoFocus
          />
        </FieldShell>
        <FieldShell
          icon={Network}
          label={t("admin:channels.field_medium")}
          htmlFor="med"
        >
          <Input
            id="med"
            ...
            placeholder={t("admin:channels.field_medium_placeholder")}
            required
          />
        </FieldShell>
      </div>
      ...
      <FieldShell
        icon={LayoutGrid}
        label={t("admin:channels.field_channel_group")}
        htmlFor="grp"
      >
        <Input
          id="grp"
          ...
          placeholder={t("admin:channels.field_channel_group_placeholder")}
          required
        />
      </FieldShell>

      <FieldShell label={t("admin:channels.field_notes")} htmlFor="notes">
        <Input
          id="notes"
          value={notes}
          onChange={(e) => setNotes(e.target.value)}
          placeholder={t("admin:channels.field_notes_placeholder")}
        />
      </FieldShell>

      <div className="flex items-start gap-2 rounded-lg border border-primary/20 bg-primary/[0.04] px-3 py-2.5 text-xs text-primary">
        <Info className="mt-0.5 size-4 shrink-0" />
        <p className="leading-relaxed">{t("admin:channels.info_hint")}</p>
      </div>
...
function ChannelMappingEditForm({
  mapping,
  onSubmit,
  loading,
  onCancel,
}: {
  mapping: ChannelMappingItem;
  onSubmit: (p: ChannelMappingUpdate) => void;
  loading: boolean;
  onCancel: () => void;
}) {
  const { t } = useTranslation(["admin", "common"]);
  const [group, setGroup] = useState(mapping.channel_group);
  ...
  return (
    <form
      ...
      className="space-y-4"
    >
      {/* Source/medium read-only (eşleme anahtarı değiştirilemez) */}
      <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border bg-surface-2/50 px-3 py-2.5 text-xs">
        <CodeChip>{mapping.source}</CodeChip>
        <span className="text-text-dim">/</span>
        <CodeChip>{mapping.medium}</CodeChip>
      </div>
      ...
      <FieldShell label={t("admin:channels.field_notes")} htmlFor="edit-notes">
        <Input
          id="edit-notes"
          value={notes}
          onChange={(e) => setNotes(e.target.value)}
