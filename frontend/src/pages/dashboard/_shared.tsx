...
function PresetButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex h-9 items-center rounded-md border border-border/80 bg-card px-3 text-xs font-semibold text-text-muted shadow-theme-xs transition-colors hover:bg-muted hover:text-foreground"
    >
      {children}
    </button>
  );
...
export function PageShell({ children }: { children: ReactNode }) {
  return (
    <div className="container mx-auto max-w-[1480px] space-y-6 px-4 py-5 sm:px-6 lg:px-8">
      {children}
    </div>
  );
}
